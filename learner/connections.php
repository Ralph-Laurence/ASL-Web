<?php
    ...
    $list = [];
    $res = $db->query_get_with_options("SELECT * FROM transactions WHERE status = 1 AND learner_id = ?", [$_SESSION["logged_in_user_id"]], ["list" => true]);
    if($res){
            
        foreach($res as $transdata){
            $tutorAccountDetails = $db->query_get("SELECT * FROM users WHERE status = 1 AND id = ?", [$transdata["tutor_id"]]);
            $list[] = $tutorAccountDetails;
        }
    }
...
<head>
    ...
    <title>Connections - Sign Lingua Learner</title>
